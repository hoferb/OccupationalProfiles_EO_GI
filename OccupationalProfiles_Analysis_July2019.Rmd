...
```{r session_info}
#devtools::session_info(include_base = TRUE, echo=FALSE)
```
\newpage
...
unique(profiles$duty)
unique(profiles$duty.man)
...
unique(profiles$original.task.name)
unique(profiles$task.man)
...
```
```{r calculate frequency of duties, echo=FALSE}
...
```{r generate single task collection for each duty}
